...
| `keepCharPositions` |  `bool`  |    -     |                -                | `false` | When true, adding or deleting characters will not affect the positions of existing characters. When false, adding characters causes existing characters to move to the right of the cursor. Deleting characters causes existing characters to move left following the cursor. Can be used only with `mask` prop |
